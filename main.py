...
#all access_keys should be your own keys
# AWS Configuration
aws_access_key_id = ''
...
def generate_concepts(user_profile):
    """
    Generates search concepts for exercise recommendations based on a user profile.
    """
    query_prompt = f"""
...
def download_video(url):
    """
    Downloads the first video from the S3 bucket using the provided URL.
    """
...
if video_urls:
    download_video(video_urls[0])
else:
    print("No video URLs available to download.")
